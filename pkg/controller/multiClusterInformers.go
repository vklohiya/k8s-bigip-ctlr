...
func (ctlr *Controller) stopDeletedGlobalCMMultiClusterInformers() error {
	...
	ctlr.multiClusterHandler.Lock()
	// remove the  pool informers for clusters whose config has been removed
	for clusterName, InfSet := range ctlr.multiClusterHandler.ClusterConfigs {
		...
		if config, ok := ctlr.multiClusterHandler.ClusterConfigs[clusterName]; !ok {
			for ns, nsPoolInf := range InfSet.comInformers {
				nsPoolInf.stop()
				delete(ctlr.multiClusterHandler.ClusterConfigs[clusterName].comInformers, ns)
			}
		} else {
			...
			if ctlr.discoveryMode == DefaultMode && !config.clusterDetails.ServiceTypeLBDiscovery {
				...
				if clusterName == ctlr.multiClusterHandler.HAPairClusterName || clusterName == ctlr.multiClusterHandler.LocalClusterName || clusterName == "" {
					continue
				} else {
					for ns, nsPoolInf := range InfSet.comInformers {
						nsPoolInf.stop()
	...
	ctlr.multiClusterHandler.Unlock()
