...
func (am *AS3Manager) getPartitions() []string {
	var partition []string
	for _, rsCfg := range am.Resources.RsMap {
		partition = append(partition, rsCfg.GetPartition())
	}
	return partition
}

func (am *AS3Manager) getPriorityTenant() []string {
	var partition []string
	for _, rsCfg := range am.Resources.RsMap {
		if rsCfg.MetaData.Priority > 0 {
			partition = append(partition, rsCfg.GetPartition())
		}
	}
	return partition
